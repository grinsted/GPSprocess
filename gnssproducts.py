...
def loaddatacenterurls():
    ...
    for i, row in datacenterurls.iterrows():
        ...
        if '%M' in upperurl: interval = datetime.timedelta(days=28)
        if '%W' in upperurl: interval = datetime.timedelta(days=7)
        if '%D' in upperurl: interval = datetime.timedelta(days=1)
